...
def obstacle_avoidance_pushing_ode():
    ...
    NeuralODE_model = NeuralODE(ode_pth_path, proj_pth_path, DEVICE).to(DEVICE)
    ...
    # val_loss_i = val_step(NeuralODE_model, val_loader, pose_loss)
    ...
    # Evaluate state
    # plt.close(fig)
    Image(filename=visualizer.get_gif(given_name='ode_obs_avoid_viso.gif'))
