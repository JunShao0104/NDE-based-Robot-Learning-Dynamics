...
class NeuralODE(nn.Module):
  def __init__(self, ode_pth_path, proj_pth_path, device = 'cpu'):
    super().__init__()
    self.device = device
  ...
  def forward(self, state, action):
      ...
      t = torch.arange(20).float().to(self.device) # (10, )
      next_state_action = odeint(self.odefunc, state_action, t) # (10, B, 3)
      # print(next_state_action.reshape(-1, next_state_action.shape[1]).shape) # 10*1000*6
