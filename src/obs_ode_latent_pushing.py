...
def obstacle_avoidance_pushing_ode():
    ...
    ode_pth_path = os.path.join(ckpt_path, 'latent_ode_single_step_t_2.pt')
    NeuralODE_model.load_state_dict(torch.load(ode_pth_path))

    controller = PushingController(env, NeuralODE_model,
                                obstacle_avoidance_pushing_cost_function, num_samples=6000, horizon=30, device=DEVICE)
    env.reset()
    ...
    # Evaluate state
    # plt.close(fig)
    name = 'obs_ode_latent_pushing_3.gif'
