...
# Process dataset
from src.dataset.data_preprocessing import process_data_single_step_FK, process_data_multiple_step_FK
...
from src.dataset.loss import SingleStepLoss, MultiStepLoss
...
# Training function
def discrete_train(model, dataset, path, num_steps=4):
    # Model
    if model == 'absolute':
    ...
    # Data loader
    train_loader, val_loader = process_data_multiple_step_FK(dataset, num_steps=num_steps) # batchsize default to be 1000

    # Loss function
    state_loss = torch.nn.MSELoss()
    state_loss = MultiStepLoss(state_loss)
...
# Training function
def continuous_train(method, dataset, path, num_steps=4):
    # dimension
    state_dim = 14
    action_dim = 7
    ...
    # Loss function
    state_loss = torch.nn.MSELoss()
    state_loss = MultiStepLoss_ode(state_loss)
    ...
    # save model
    save_path = os.path.join(ckpt_path, 'ODEFunc_multi_step_{}_{}.pt'.format(method if method else "dopri5", num_steps))
