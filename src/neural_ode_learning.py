...
def train_singlestep(method, dataset, path):
    ...
    # save model:
    ode_model_save_path = os.path.join(ckpt_path, 'ODEFunc_single_step_{}_func3_0_1.pt'.format(method if method else "dopri5"))
...
def train_multistep(method, dataset, path):
    # dimension
    state_dim = 3
    action_dim = 3
    num_steps = 8
